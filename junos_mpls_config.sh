...
usage() {
    ...
    echo ""
    echo "Examples:"
    echo "  $0                                 # Use all defaults (rsvp, ospf, ecmp disabled)"
...
if [[ "$IGP_PROTOCOL" == "isis" ]]; then
    ...
    ansible-playbook playbooks/juniper/ospf.yml -e 'group=acx1100_logical_systems' --forks 1
fi
((STEP++))
...
if [[ "$LABEL_PROTOCOL" == "ldp" ]]; then
    ...
    ansible-playbook playbooks/juniper/rsvp.yml -e 'group=acx1100_logical_systems' --forks 1
fi
((STEP++))
